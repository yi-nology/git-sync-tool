<template>
  <AppPage title="通知渠道管理">
    <AppCard>
      <div class="notification-manager">
        <!-- 工具栏 -->
        <div class="toolbar">
          <AppButton type="primary" icon="Plus" @click="openAddDialog">
            添加渠道
          </AppButton>
          <AppButton type="secondary" icon="Refresh" @click="loadChannels" :disabled="loading">
            刷新
          </AppButton>
        </div>
        ...
        <!-- 添加/编辑渠道对话框 -->
        <el-dialog v-model="showDialog" :title="editingId ? '编辑通知渠道' : '添加通知渠道'" width="640px" destroy-on-close append-to-body>
...
.template-input.editor-active :deep(.el-input__wrapper) {
  box-shadow: 0 0 0 1px var(--primary-color) inset, 0 0 0 3px var(--accent-bg);
}
