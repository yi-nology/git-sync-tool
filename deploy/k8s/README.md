...
| 命令 | 功能 | 说明 |
|------|------|------|
| `./deploy.sh deploy` | 部署应用 | 自动部署所有组件，包括前端资源上传 |
| `./deploy.sh uninstall` | 卸载应用 | 删除所有资源，可选保留 PVC |
| `./deploy.sh restart` | 重启应用 | 重启所有 Pod |
| `./deploy.sh status` | 查看状态 | 显示所有资源状态 |
| `./deploy.sh logs` | 查看日志 | 实时查看应用日志 |
| `./deploy.sh update-frontend` | 更新前端 | 更新前端资源并重启 Nginx |
| `./deploy.sh scale` | 扩缩容 | 调整副本数 |
| `./deploy.sh help` | 帮助信息 | 显示所有命令 |

**前置条件**：
- 已安装 kubectl 并配置好集群连接
- 已构建前端资源：`make build-frontend-integrate`
